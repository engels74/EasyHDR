...
impl GuiController {
    ...
    pub fn new(
        ...
        // Set window icon
        #[cfg(windows)]
        Self::set_window_icon(&main_window);
    ...
    /// Set the window icon from the embedded application icon
    ///
    /// Loads the application icon from `assets/icon.ico` and sets it as the window icon
    /// in the title bar. Uses the winit backend accessor to set the icon on the underlying
    /// winit window.
    #[cfg(windows)]
    fn set_window_icon(main_window: &MainWindow) {
        ...
        match Self::load_icon_from_bytes(icon_bytes) {
            Ok(icon) => {
                // Access the underlying winit window and set the icon
    ...
    /// Load a winit Icon from raw icon bytes
    ///
    /// Decodes the icon file (ICO format) and converts it to a winit Icon.
    /// Uses the `image` crate to decode the ICO file and extract the RGBA data.
    #[cfg(windows)]
    fn load_icon_from_bytes(icon_bytes: &[u8]) -> Result<slint::winit_030::winit::window::Icon> {
        use easyhdr::error::EasyHdrError;
        use image::GenericImageView;
        use std::io::Cursor;

        // Decode the ICO file
        let img = image::load(Cursor::new(icon_bytes), ImageFormat::Ico)
