...
impl WindowsVersion {
    ...
    /// # Safety
    ///
    /// `LoadLibraryW` called with null-terminated wide string for "ntdll.dll" (guaranteed
    /// to exist). `GetProcAddress` validated for non-None before use. Transmute from
    /// `Option<FARPROC>` to `RtlGetVersionFn` sound: signature matches API contract,
    ...
    /// # Safety
    ///
    /// `OSVERSIONINFOEXW` initialized with correct size to prevent buffer overruns.
    /// `addr_of_mut!().cast()` creates valid aligned pointer to stack variable.
    /// Return value checked via `is_ok()` before data access. Failures propagate
    /// error via `Error::from_thread()`, preventing invalid reads.
    #[cfg(windows)]
    #[expect(
        unsafe_code,
